...
Another optimization would be to dynamically limiting the bandwidth based on heuristics. For example, if the number of messages and peers increase a certain threshold, the bandwidth can be reduced by increasing the timeframe of the semaphore. 
...
Instead of using a predetermined structure, we decided to use an more open-approach that allows for easy modifiability. Hence, we are using a Key-Value store, which will be serialized and deserialized. 
...
public static interface Command {
		...
		public enum Info {
			Created, Deleted, Modified, Heartbeat, Leave;
		}
	}
\end{lstlisting}
